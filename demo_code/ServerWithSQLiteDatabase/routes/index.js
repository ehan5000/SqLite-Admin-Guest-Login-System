...
exports.users = function(request, response) {
  ...
  if (request.user_role !== 'admin') {
    ...
    response.end('ERROR: Admin Privliges Required To See Users');
    return;
  }
...
exports.find = function(request, response) {
  ...
  let keySql = urlObj.query['title'].replace(/\+/g,' ');
  let sql = "SELECT id, title FROM songs"

  if (urlObj.query['title']) {
    console.log("finding title: " + urlObj.query['title']);
    let keyWord = '%'+keySql.split(' ').join('%')+'%';
    sql = "SELECT id, title FROM songs WHERE title LIKE '" +keyWord + "'";
  }
